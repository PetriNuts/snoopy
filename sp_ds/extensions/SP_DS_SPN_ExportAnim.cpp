...
#include<wx/filefn.h>
#include "SP_DS_SPN_ExportAnim.h"
...
BEGIN_EVENT_TABLE(ExportSPN, wxFrame)
EVT_BUTTON(wxID_OK, ExportSPN::OnOK)
EVT_BUTTON(wxID_CANCEL, ExportSPN::OnExit)
EVT_RADIOBOX(wxID_RADIO_CHOICE, ExportSPN::OnRadioChoice)
END_EVENT_TABLE()

ExportSPN::ExportSPN(const wxString &title, SP_DS_StAnimation *temp) :
...
void ExportSPN::OnRadioChoice(wxCommandEvent &event)
{
...
void ExportSPN::OnOK(wxCommandEvent &event)
{
	if (m_pc_Animator)
	{
...
void ExportSPN::OnExit(wxCommandEvent &event)
{
	Close();
}
