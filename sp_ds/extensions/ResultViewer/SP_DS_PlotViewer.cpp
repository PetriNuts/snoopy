...
SP_DS_PlotViewer::SP_DS_PlotViewer(wxWindow* p_pcParent, wxSizer* p_pcSizer) :
		SP_DS_ResultViewer(p_pcParent, p_pcSizer), m_pcChartPanel(NULL),
				m_bShowLegend(true), m_nLegendHorizontalPosition(wxRIGHT), m_nLegendVerticalPosition(wxCENTER),
				m_sXAxisTitle(wxT("")), m_sYAxisTitle(wxT("")),
		m_bShowXAxis(true), m_bShowYAxis(true),
		m_nCurveOrder(0), m_bFixedXAdjustment(false), m_bFixedYAdjustment(false),
		m_nMinXValue(0), m_nMaxXValue(1), m_nMinYValue(0), m_nMaxYValue(1)
{
	...
	m_sSupportedSaveExtensions = wxT("BMP (*.bmp)|*.bmp | GIF (*.gif)|*.gif | PNG (*.png)|*.png | JPG (*.jpg)|*.jpeg "); //| Postscript(*.ps)|*.ps");
...
void SP_DS_PlotViewer::Update()
{
	...
	//remove background
	l_pcChart->SetBackground(new NoAreaDraw());
...
void SP_DS_PlotViewer::UpdateAttributes()
{
	...
	{
		m_nLegendHorizontalPosition = GetPosition((dynamic_cast<SP_DS_ViewerAttributeList*>(GetAttributeByName(wxT("LEGEND_HORIZONTAL_POS"))))->GetSelection());
	}

	//Legend position(Vertical)
	if (GetAttributeByName(wxT("LEGEND_VERTICAL_POS")) != NULL)
	{
		m_nLegendVerticalPosition = GetPosition((dynamic_cast<SP_DS_ViewerAttributeList*>(GetAttributeByName(wxT("LEGEND_VERTICAL_POS"))))->GetSelection() + 3);
	}
	...
	{
		m_nCurveOrder = (dynamic_cast<SP_DS_ViewerAttributeList*>(GetAttributeByName(wxT("CURVE_SORT"))))->GetSelection();
...
void SP_DS_PlotViewer::LoadCurrentAttributeValues()
{
	...
	if (GetAttributeByName(wxT("LEGEND_HORIZONTAL_POS")) != NULL)
	{
		...
		(dynamic_cast<SP_DS_ViewerAttributeList*>(GetAttributeByName(wxT("LEGEND_HORIZONTAL_POS"))))->SetSelection(l_nPosition);
	}
	...
	if (GetAttributeByName(wxT("LEGEND_VERTICAL_POS")) != NULL)
	{
		...
		(dynamic_cast<SP_DS_ViewerAttributeList*>(GetAttributeByName(wxT("LEGEND_VERTICAL_POS"))))->SetSelection(l_nPosition);
	}
	...
	if (GetAttributeByName(wxT("CURVE_SORT")) != NULL)
	{
		(dynamic_cast<SP_DS_ViewerAttributeList*>(GetAttributeByName(wxT("CURVE_SORT"))))->SetSelection(m_nCurveOrder);
...
void SP_DS_PlotViewer::ExportToImageFile(wxString& p_sFileName, const int& p_nFileType)
{
	...
	// we replace the background color (gray - 220,220,220) with white
	//SP_LOGMESSAGE(wxString::Format(wxT("color at (0,0): r(%i)g(%i)b(%i)"), image.GetRed(0,0), image.GetGreen(0,0), image.GetBlue(0,0)));
	//image.Replace(image.GetRed(0,0), image.GetGreen(0,0), image.GetBlue(0,0), 255,255,255);
	...
	image.SaveFile(p_sFileName, l_nExportType);
}
