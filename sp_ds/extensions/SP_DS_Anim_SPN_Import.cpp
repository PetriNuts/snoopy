...
#include<wx/filefn.h>
#include "SP_DS_Anim_SPN_Import.h"

BEGIN_EVENT_TABLE(ImportSPN, wxFrame)
EVT_BUTTON(wxID_CANCEL, ImportSPN::OnExit)
EVT_BUTTON(wxID_OK, ImportSPN::OnOK)
END_EVENT_TABLE()

ImportSPN::ImportSPN(const wxString &title, class SP_DS_StAnimation *temp) :
...
void ImportSPN::OnExit(wxCommandEvent &event)
{
	Close();
}

void ImportSPN::OnOK(wxCommandEvent &event)
{
