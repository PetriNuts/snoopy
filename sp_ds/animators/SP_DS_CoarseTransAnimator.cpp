...
SP_DS_CoarseTransAnimator::SP_DS_CoarseTransAnimator(SP_DS_Animation* p_pcAnim, SP_DS_Node* p_pcParent):
...
SP_DS_CoarseTransAnimator::~SP_DS_CoarseTransAnimator()
{
    if (m_pcProtoshape)
...
SP_DS_Animator*
SP_DS_CoarseTransAnimator::Clone(SP_Data* p_pcParent, SP_DS_Animation* p_pcAnim)
{
    if (p_pcParent->GetElementType() != SP_ELEMENT_NODE)
        return NULL;

    SP_DS_CoarseTransAnimator* l_pcNew = new SP_DS_CoarseTransAnimator(p_pcAnim, dynamic_cast<SP_DS_Node*>(p_pcParent));

    return dynamic_cast<SP_DS_Animator*>(l_pcNew);
}
#if 0
bool
SP_DS_CoarseTransAnimator::InformPrePlaces()
{
    Reset();
...
bool
SP_DS_CoarseTransAnimator::TestConcession()
{
...
bool
SP_DS_CoarseTransAnimator::Reset()
{
    map<SP_DS_PlaceAnimator*, SP_ListEdge >::iterator l_Iter;
...
// animate
bool
SP_DS_CoarseTransAnimator::PreStep(unsigned int p_nSteps, SP_DIRECTION_T p_nDir)
{
    if (!m_pcNode || !m_pcAnimation)
        return FALSE;
...
bool
SP_DS_CoarseTransAnimator::Step(unsigned int p_nStep)
{
    if (!GetGraphics())
        return TRUE;
...
bool
SP_DS_CoarseTransAnimator::PostStep()
{
    SP_ListEdge::const_iterator l_itEdges;
    SP_DS_PlaceAnimator* l_pcAnim;
...
#endif
bool
SP_DS_CoarseTransAnimator::OnLeftClick()
{
...
bool
SP_DS_CoarseTransAnimator::OnRightClick()
{
    CHECK_POINTER(m_pcNode, return FALSE);
    CHECK_POINTER(m_pcAnimation, return FALSE);
    SP_DS_PedAnimation* l_pcAnim = dynamic_cast<SP_DS_PedAnimation*>(m_pcAnimation);
...
#if 0
void
SP_DS_CoarseTransAnimator::Enable(SP_DS_PlaceAnimator* p_pcAnimator)
{
