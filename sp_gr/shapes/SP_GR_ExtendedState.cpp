...
//
//---------------------------------------------------------------
SP_GR_ExtendedState::SP_GR_ExtendedState(SP_DS_Node* p_pcParent,double p_nWidth,double p_nHeight,
...
SP_GR_ExtendedState::~SP_GR_ExtendedState()
{
	if (m_pcPrimitive)
    {
...
SP_Graphic*
SP_GR_ExtendedState::Clone(SP_Data* p_pcParent)
{
    if (p_pcParent->GetElementType() != SP_ELEMENT_NODE)
        return NULL;

    SP_GR_ExtendedState* l_pcNew = new SP_GR_ExtendedState(dynamic_cast<SP_DS_Node*>(p_pcParent),
...
SP_GR_ExtendedState::Draw()
{
  ...
  int l_nRadius = 0;
  if(l_nX<=l_nY) {
	 l_nRadius= wxRound(l_nX / 2);
  } else
  {
	  l_nRadius= wxRound(l_nY / 2);
  }
...
*/
  ...
  m_pcPrimitive->SetDrawnBrush(GetBrush(), TRUE);
	...
	if (SP_EXTENDED_TYPE_TWO == l_nType) 
	{
		...
		m_pcPrimitive->DrawRoundedRectangle(wxRect(-wxRound( l_nX * 0.75),-wxRound(l_nY * 0.75),wxRound( l_nX * 1.5) ,wxRound( l_nY * 1.5)),l_nRadius);
	}
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
					//SP_EXTENDED_TYPE_DEFAULT and all not define 
