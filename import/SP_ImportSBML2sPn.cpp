...
bool SP_ImportSBML2sPn::ReadFile(const wxString& p_sFile,SP_ImportRoutine* p_sIR)
{
	m_Species.clear();
	m_Reactions.clear();
...
void SP_ImportSBML2sPn::getSpecies()
{
    ...
    for (unsigned int n = 0; n < m_sbmlModel->getNumSpecies() ; ++n)
    {
		...
		if (l_pcNode)
		{
			...
			if(l_sbmlSpecies->getBoundaryCondition())
			{
				l_comment << wxT("boundaryCondition=\"true\"\n");
			}
			if(l_sbmlSpecies->getConstant())
			{
				l_comment << wxT("constant=\"true\"\n");
			}
...
void SP_ImportSBML2sPn::getEvents()
{
    for (unsigned int n = 0; n < m_sbmlModel->getNumEvents() ; ++n)
    {
        ...
        getSBMLName(l_sbmlEvent, l_eventId, l_eventName);
		...
		if (l_pcNode)
		{
			...
			for(unsigned int k = 0; k < l_sbmlEvent->getNumEventAssignments(); ++k)
			{
				...
				if ( l_sbmlEventAssignment->isSetMath() )
				{
					...
					SP_DS_Node* l_pcPlace = getSpeciesNode(variable);
					if(l_pcPlace)
					{
	...
	for (unsigned int i = 0; i < m_sbmlModel->getNumReactions() ; ++i)
    {
		...
		if (l_reactionNode)
		{
			...
			for (unsigned int lor = 0; lor < rectants->size() ; ++lor)
			{
				...
				{
                    l_pcEdge = drawEdge(l_revReactionNode,l_pcNode,SP_DS_EDGE,l_stoichiometry);
 					l_pcEdge->GetAttribute(wxT("Comment"))->SetValueString(l_comment);
					if(m_NormalizeStoichiometries)
					{
						l_Stoichiometries.insert(std::make_pair(l_pcEdge, l_sbmlStoichiometry));
			...
			for (unsigned int lop = 0; lop < products->size() ; ++lop)
			{
				...
				if (b_IsReversible && l_revReactionNode && m_CreateReverseReactions)
				{
					l_pcEdge = drawEdge(l_pcNode, l_revReactionNode, SP_DS_EDGE,l_stoichiometry);
					l_pcEdge->GetAttribute(wxT("Comment"))->SetValueString(l_comment);
					if(m_NormalizeStoichiometries)
					{
						l_Stoichiometries.insert(std::make_pair(l_pcEdge, l_sbmlStoichiometry));
...
void SP_ImportSBML2sPn::getFunctionDefinitions() {
