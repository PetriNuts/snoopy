...
void SP_DLG_FpnConstantDefinition::Operate(const unsigned& p_nReason, const unsigned& r)
{
	...
	if (p_nReason == 1)
	{
		...
		menu->Connect(wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(SP_DLG_FpnConstantDefinition::OnPopupClick), NULL, this);
...
void SP_DLG_FpnConstantDefinition::OnPopupClick(wxCommandEvent& evt)
{
	int l_nToRow = 0;
	l_nToRow = evt.GetId();
	l_nToRow = l_nToRow - 1;
