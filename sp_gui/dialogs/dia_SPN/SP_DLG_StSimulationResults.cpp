...
enum
{
	SP_ID_CHOICE_SIMULATOR_CHOICE = SP_ID_LAST_ID + 100,
...
EVT_CHOICE(SP_ID_CHOICE_SIMULATOR_CHOICE, SP_DLG_StSimulationResults::OnSimulatorSelect)
...
: SP_DLG_Simulation(p_pcGraph, p_pcParent, p_sHelpText, p_sTitle, p_nStyle)
{
	...
	if(!m_pcGraph->GetNetclass()->GetName().Contains(wxT("Colored")))
	{
		...
		for(l_itChoice = m_choices.begin(); l_itChoice != m_choices.end(); ++l_itChoice)
		{
			...
			m_apcComboBoxes.push_back(new wxChoice( m_pcPropertyWindowSetsSizer, -1 , wxDefaultPosition, wxSize(100,-1), 0, NULL, wxCB_READONLY, wxDefaultValidator, l_sGroup ));
			l_pcRowSizer->Add( m_apcComboBoxes[m_apcComboBoxes.size()-1], wxSizerFlags(1).Expand().Border(wxALL, 2));
...
void SP_DLG_StSimulationResults::UpdateViewer(SP_DS_Metadata* p_pcView)
{
	CHECK_POINTER(m_pcMainSimulator, return);
...
void SP_DLG_StSimulationResults::UpdateSimulationMatrix(SP_DS_Metadata* p_pcView)
{
