...
void SP_GUI_SteeringDialog::ShowSimulationIntervals()
{
	...
	{
		m_pcIntervalStartTextCtrl->SetValue(dssd::aux::toString(m_nOutputStartPoint));
	}

	if (m_pcIntervalEndTextCtrl->IsModified() == false)
	{
		m_pcIntervalEndTextCtrl->SetValue(dssd::aux::toString(m_nOutputEndPoint));
	}

	if (m_pcResultPointCountTextCtrl->IsModified() == false)
	{
		m_pcResultPointCountTextCtrl->SetValue(dssd::aux::toString(m_nOutputSamplingSize));
	}
}
...
void SP_GUI_SteeringDialog::UpdateStatistics()
{
	...
	m_pcSimulationProgressPercentage->SetLabel(dssd::aux::toString(l_nSimulatorProgress) + wxT("%"));
...
void SP_GUI_SteeringDialog::OnExport2CSVSelected(wxCommandEvent& WXUNUSED(event))
{
	...
	SP_DLG_StDirectExportProperties* l_pcDlg = new SP_DLG_StDirectExportProperties(SP_ST_SIM_EXPORT_CSV_EDIT_DLG_EXPLICIT, this, &m_sExportFilename, &m_nExportSpacer, &l_bCompressExact, &l_bCompressExact);
