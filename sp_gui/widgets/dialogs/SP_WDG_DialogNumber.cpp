...
#include "sp_gui/widgets/dialogs/SP_WDG_DialogNumber.h"

SP_WDG_DialogNumber::SP_WDG_DialogNumber(const wxString& p_sPage, int p_nMin, int p_nMax, unsigned int p_nOrdering)
...
SP_WDG_DialogNumber::~SP_WDG_DialogNumber()
{
}

SP_WDG_DialogBase*
SP_WDG_DialogNumber::Clone()
{
    return new SP_WDG_DialogNumber(GetName(), m_nMin, m_nMax, GetDialogOrdering());
}

bool
SP_WDG_DialogNumber::AddToDialog(const SP_ListAttribute* p_ptlAttributes, 
                               ...
                               bool p_bOverview)
{
    ...
    if(p_bOverview)
    {
    	...
    	for(; it != end; ++it)
    	{
	        ...
	        l_pcSizer->Add(l_pcSpinCtrl, 1, wxALL , 5);
    ...
    else
    {
    	...
    	wxString l_pchValue = (m_bMultiple ? wxString() : l_pcAttr->GetValueString());
...
SP_WDG_DialogNumber::OnDlgOk()
{
    ...
    else
    {
    	if (m_bMultiple)
    	{
    		if (m_pcSpinCtrl[0]->GetValue() == INT_MIN)
    		{
