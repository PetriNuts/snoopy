...
SP_WDG_DialogShowOnly::SP_WDG_DialogShowOnly(const wxString& p_sPage, unsigned int p_nOrdering)
:SP_WDG_DialogBase(p_sPage, p_nOrdering)
{
}

SP_WDG_DialogShowOnly::~SP_WDG_DialogShowOnly()
{
}

SP_WDG_DialogBase*
SP_WDG_DialogShowOnly::Clone()
{
    return new SP_WDG_DialogShowOnly(GetName(), GetDialogOrdering());
}

bool
SP_WDG_DialogShowOnly::AddToDialog(const SP_ListAttribute* p_ptlAttributes, 
                               ...
                               bool p_bOverview)
{
    ...
    else
    {
		...
		l_pcSizer->Add(new wxStaticText(l_pcPage, -1, l_sLabel), 1, wxALL , 5);
		AddShowFlag(l_pcPage, l_pcSizer, l_pcAttr);
