...
SP_ExportColPT2PT::SP_ExportColPT2PT()
{	
}
...
bool SP_ExportColPT2PT::AcceptsDoc(SP_MDI_Doc* p_doc)
{
...
bool SP_ExportColPT2PT::Write(SP_MDI_Doc* p_doc, const wxString& p_fileName)
{
	CHECK_POINTER(p_doc, return false);
	CHECK_POINTER(p_doc->GetGraph(), return false);
...
bool SP_ExportColPT2PT::WriteNetclass(SP_DS_Netclass* p_pcVal, wxXmlNode* p_pcRoot)
{
...
bool SP_ExportColPT2PT::WriteNodeclass(SP_DS_Nodeclass* p_pcVal, wxXmlNode* p_pcRoot )
{
	CHECK_POINTER( p_pcVal, return FALSE );
	CHECK_POINTER( p_pcRoot, return FALSE );	
...
bool SP_ExportColPT2PT::WriteEdgeclass(SP_DS_Edgeclass* p_pcVal, wxXmlNode* p_pcRoot)
{	
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
...
bool SP_ExportColPT2PT::WriteEdge(SP_DS_Edge* p_pcVal,wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
    CHECK_POINTER(p_pcRoot, return FALSE);
...
bool SP_ExportColPT2PT::WriteUnfoldedPlace(SP_DS_Node* p_pcVal, wxXmlNode* p_pcRoot )
{	
    CHECK_POINTER(p_pcVal, return FALSE);
    CHECK_POINTER(p_pcRoot, return FALSE);
...
bool SP_ExportColPT2PT::WriteMetadataclass( SP_DS_Metadataclass* p_pcVal, wxXmlNode* p_pcRoot )
{
	CHECK_POINTER( p_pcVal, return FALSE );
	CHECK_POINTER( p_pcRoot, return FALSE );
		...
		for (size_t l_nCon = 0; l_nCon < l_pcCOnstants->GetElements()->size(); l_nCon++)
		{
			if ((*l_itElem)->GetAttribute(wxT("Group"))->GetValueString() == wxT("coloring"))
			   {//12.2020, constants belonging to the colouring group should not be exported to the uncolored net
					++l_itElem;
					if (!l_bIScolouringGroup)//to make this warning ocuur once only
					  {
...
bool SP_ExportColPT2PT::WriteUnfoldedTransition(SP_DS_Node* p_pcVal, wxXmlNode* p_pcRoot )
{	
    CHECK_POINTER(p_pcVal, return FALSE);
    CHECK_POINTER(p_pcRoot, return FALSE);
