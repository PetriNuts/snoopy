...
bool SP_ExportContPed2Ped::Write(SP_MDI_Doc* p_doc, const wxString& p_fileName)
{
...
bool SP_ExportContPed2Ped::WriteNetclass(SP_DS_Netclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
...
bool SP_ExportContPed2Ped::WriteNodeclass(SP_DS_Nodeclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
		...
		wxXmlNode* p_pcRoot)
{
	...
	l_pcElem->AddAttribute(wxT("count"),wxString::Format( wxT("%d"), l_nCount));
