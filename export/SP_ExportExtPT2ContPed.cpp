...
bool SP_ExportExtPT2ContPed::Write(SP_MDI_Doc* p_doc,
...
bool SP_ExportExtPT2ContPed::WriteNetclass(SP_DS_Netclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
...
bool SP_ExportExtPT2ContPed::WriteNodeclass(SP_DS_Nodeclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
...
bool SP_ExportExtPT2ContPed::WriteEdgeclass(SP_DS_Edgeclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
	...
	else if (wxT("Reset Edge") == l_sEdgeclassName)
	{
		l_pcEdgeclassName = SP_DS_EDGE;
...
bool SP_ExportExtPT2ContPed::WriteEdge(SP_DS_Edge* p_pcVal,
		wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
...
bool SP_ExportExtPT2ContPed::AcceptsDoc(SP_MDI_Doc* p_doc)
{
	CHECK_POINTER(p_doc, return false);
	CHECK_POINTER(p_doc->GetGraph(), return false);
	wxString cName = p_doc->GetNetclassName();
	return (SP_DS_EXTPN_CLASS == cName);
}
