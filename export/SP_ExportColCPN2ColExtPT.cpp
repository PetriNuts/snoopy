...
bool SP_ExportColCPN2ColExtPT::Write(SP_MDI_Doc* p_doc,
...
bool SP_ExportColCPN2ColExtPT::WriteNetclass(SP_DS_Netclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
...
bool SP_ExportColCPN2ColExtPT::WriteNodeclass(SP_DS_Nodeclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
	...
	{
		l_sNodeclassName = SP_DS_DISCRETE_TRANS;
		l_pcElem->AddAttribute(wxT("name"), l_sNodeclassName);
...
bool
SP_ExportColCPN2ColExtPT :: WriteStochPlace( SP_DS_Node* p_pcVal, wxXmlNode* p_pcRoot)
{
  ...
  SP_DS_Attribute* l_pcMarking = NULL;
    ...
    l_pcMarking->SetValueString( l_pcColList->GetCell(0,l_nActiveColumn+1) );
...
bool
SP_ExportColCPN2ColExtPT :: WriteStochTransition( SP_DS_Node* p_pcVal, wxXmlNode* p_pcRoot)
{
...
/*
bool
SP_ExportColCPN2ColExtPT :: WriteStochParameter( SP_DS_Node* p_pcVal, wxXmlNode* p_pcRoot)
{
...
bool SP_ExportColCPN2ColExtPT::WriteEdgeclass(SP_DS_Edgeclass* p_pcVal,
		wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
	...
	{
		l_pcEdgeclassName = SP_DS_INHIBITOR_EDGE;
	}
	else if (SP_DS_READ_EDGE == l_sEdgeclassName)
	{
		l_pcEdgeclassName = SP_DS_READ_EDGE;
	}
	else
	{
		return true;
...
bool
SP_ExportColCPN2ColExtPT::WriteEdge(SP_DS_Edge* p_pcVal, wxXmlNode* p_pcRoot)
{
    CHECK_POINTER(p_pcVal, return FALSE);
    CHECK_POINTER(p_pcRoot, return FALSE);
...
bool SP_ExportColCPN2ColExtPT::AcceptsDoc(SP_MDI_Doc* p_doc)
{
	CHECK_POINTER(p_doc, return false);
	CHECK_POINTER(p_doc->GetGraph(), return false);
