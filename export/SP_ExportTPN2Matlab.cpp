...
SP_ExportTPN2Matlab::SP_ExportTPN2Matlab()
{
}
...
bool SP_ExportTPN2Matlab::AcceptsDoc(SP_MDI_Doc* p_doc)
{
...
bool SP_ExportTPN2Matlab::DoWrite()
{
	...
	m_file.Write(wxT("%TPN2Matlab(pn);\n"));
...
bool SP_ExportTPN2Matlab::WritePlaces()
{
...
bool SP_ExportTPN2Matlab::WriteTransitions()
{
	wxString l_sTransVector = wxT("pn.T = {");
	wxString l_sFunctions;
	wxString l_sFuncVector = wxT("pn.F = {");
	wxString l_sFuncVectorLow = wxT("pn.FLow = {");
	wxString l_sFuncVectorUp = wxT("pn.FUp = {");
	...
	for (tIt = m_trIdMap.begin(); tIt != m_trIdMap.end(); tIt++)
	{
		...
		if(!t->m_function.IsEmpty())
		{
			...
			l_sFuncVectorLow << l_sFunction.BeforeFirst(',');
			l_sFuncVectorUp << l_sFunction.AfterFirst(',');
	...
	*/
	...
	m_file.Write(l_sFuncVectorLow);
	m_file.Write(l_sFuncVectorUp);
	m_file.Write(wxT("\n"));
	return TRUE;
}

bool SP_ExportTPN2Matlab::WriteConstants()
{
...
wxString
SP_ExportTPN2Matlab::SubstituteNames(const wxString& p_Func)
{
