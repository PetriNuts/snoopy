...
bool
SP_ExportPRISM::AddToDialog(SP_DLG_ExportProperties* p_pcDlg, SP_MDI_Doc* p_pcDoc)
{
	 SP_WDG_NotebookPage* l_pcNotebookPage = p_pcDlg->AddPage(wxT("PRISM"));
...
bool SP_ExportPRISM::AcceptsDoc(SP_MDI_Doc* p_doc)
{
...
bool SP_ExportPRISM::ReadBoundedness()
{
...
bool SP_ExportPRISM::OrdinaryWrite()
{

	m_file.Write(wxT("//created by ") + SP_APP_LONG_NAME + wxT("\n"));
	m_file.Write(wxT("//date: ") + ::wxNow() + wxT("\n\n"));
	m_file.Write(wxT("ctmc\n\n"));
	m_file.Write(wxT("const int Max;\n"));
	m_file.Write(wxT("const int N;\n"));
	m_file.Write(ReadConstant());
	m_file.Write(wxT("module  ") + GetNetName(m_fileName) + wxT("\n\n"));
	WritePlaces();
	WriteTransitions();


	m_file.Write(wxT("\n\n"));
	m_file.Write(wxT("endmodule\n"));

	return !m_bError;

}
bool SP_ExportPRISM::IntelligentWrite()
{
...
bool SP_ExportPRISM::WritePlaces()
{
	...
	for (unsigned i = 0; i < l_OriginalPlace.size(); i++)
	{
		if (m_nMode == 1)
		{
			if (GetMark(l_OrderPlaces[i]) == 0)
				m_file.Write(wxString::Format(wxT("%s")wxT(":  [ 0..Max ]  init %u;\n"), l_OrderPlaces[i].c_str(), GetMark(l_OrderPlaces[i])));
			else
				m_file.Write(wxString::Format(wxT("%s")wxT(":  [ 0..Max ]  init %u*N;\n"), l_OrderPlaces[i].c_str(), GetMark(l_OrderPlaces[i])));
		}

		if (m_nMode == 2)
		{
			if (GetMark(l_OrderPlaces[i]) == 0)
				m_file.Write(wxString::Format(wxT("%s")wxT(":  [ 0..%s*N ]  init %u;\n"), l_OrderPlaces[i].c_str(), GetMax(l_OrderPlaces[i]).c_str(), GetMark(l_OrderPlaces[i])));
			else
				m_file.Write(wxString::Format(wxT("%s")wxT(":  [ 0..%s*N ]  init %u*N;\n"), l_OrderPlaces[i].c_str(), GetMax(l_OrderPlaces[i]).c_str(), GetMark(l_OrderPlaces[i])));
		}
	}
...
bool SP_ExportPRISM::WriteTransitions()
{
	...
	for (tIt = m_trIdMap.begin(); tIt != m_trIdMap.end(); tIt++)
	{
		...
		if (aList)
		{
			...
			for (aIt = aList->begin(); aIt != aList->end(); aIt++)
			{
				...
				{
					l_sBuffer1 << l_sSeperator << wxT("(") << nName << wxT(" < ") << l_pSP_DiscrArc->m_mult << wxT(")");
				}
				else if ((l_pSP_DiscrArc->m_edge->GetClassName()) == wxT("Read Edge"))
				{
					l_sBuffer1 << l_sSeperator << wxT("(") << nName << wxT(" > ") << l_pSP_DiscrArc->m_mult-1 << wxT(")");
				}
			...
			for(SP_MapString2Int::iterator l_it = l_mMultiplicity.begin(); l_it != l_mMultiplicity.end(); l_it++)
			{
				...
				l_sBuffer1 << l_sSeperator << wxT("(") << l_it->first << wxT(" > ") << ((l_it->second+1) * (-1)) << wxT(")");
		...
		if (aList)
		{
			for (aIt = aList->begin(); aIt != aList->end(); aIt++)
			{
				...
				if (m_nMode == 1)
				{
					if(l_pSP_DiscrArc->m_mult-1 == 0)
						l_sBuffer1 << l_sSeperator << wxT("(") << nName << wxT(" < Max") << wxT(" )");
					else
						l_sBuffer1 << l_sSeperator << wxT("(") << nName << wxT(" < Max-")<<l_pSP_DiscrArc->m_mult-1 << wxT(" )");
				}
				if (m_nMode == 2)
				{
					if( l_pSP_DiscrArc->m_mult-1 == 0 )
						l_sBuffer1 << l_sSeperator << wxT("(") << nName << wxT(" < ") << GetMax(nName)<<wxT("*N")<< wxT(")");
					else
						l_sBuffer1 << l_sSeperator << wxT("(") << nName << wxT(" < ") << GetMax(nName)<<wxT("*N")<< wxT("-")<<l_pSP_DiscrArc->m_mult-1<< wxT(")");
				}
		...
		//begin to write
			...
			for(SP_MapString2Int::iterator l_it = l_mMultiplicity.begin(); l_it != l_mMultiplicity.end(); l_it++)
			{
				...
				if(l_mReset.find(l_it->first) != l_mReset.end())
				{
					l_sBuffer2 << l_sSeperator << wxT("(") << l_it->first << wxT("' = ") << l_it->second << wxT(")");
				}
				else if(l_it->second < 0)
				{
					l_sBuffer2 << l_sSeperator << wxT("(") << l_it->first << wxT("' = ") << l_it->first << wxT("-") << (l_it->second * -1) << wxT(")");
				}
				else if(l_it->second > 0)
				{
					l_sBuffer2 << l_sSeperator << wxT("(") << l_it->first << wxT("' = ") << l_it->first << wxT("+") << l_it->second << wxT(")");
				}
			}
		...
		if (!l_pSP_Transition->m_function.IsEmpty())
		{
			...
			if (l_sFunction.Find(l_BMA) != -1)
			{
				...
				for (size_t i = 0; i < l_parStr.Len(); i++)
				{
						...
						l_pchCurrent == wxT("/") || l_pchCurrent == wxT("^"))
					{
						if (!l_sToken.IsEmpty())
						{
							...
							{
								l_sResultFunctionPara << l_itParam->second->m_marking;
							}
							else
							{
				...
				l_sBuffer3 << wxT(" -> (") << l_sResultFunctionPara << wxT(")") << l_sResultFunction << wxT(" : ");
				...
				/////////////////////////////////////////////
					...
					l_sBuffer3 << wxT(" -> ") << l_sResultFunctionPara << l_sResultFunction << wxT(" : ");
				}
				else
				{
					//dealing main
					...
					l_sBuffer3 << wxT(" -> ") << l_sResultFunction << wxT(" : ");
				}
		}

		//write the buffer to file
		wxString l_sBuffer4;
		l_sBuffer4 << l_sBuffer << l_sBuffer1 << wxT("\n") << l_sBuffer3 << wxT("\n") << l_sBuffer2 << wxT(";");
		l_sBuffer4 << wxT("\n\n\n");
...
wxString SP_ExportPRISM::GetPlaceName(const long PlaceID)
{
	PlaceIdMap::iterator pIt;
	SP_Place* l_pcPlace;
	wxString nName;
...
int SP_ExportPRISM::GetMark(const wxString placename)
{
	PlaceIdMap::iterator pIt;
	SP_Place* l_pcPlace;
...
wxString SP_ExportPRISM::GetMax(const wxString placename)
{
...
void SP_ExportPRISM::Reverse(vector<wxString>& vPlace)
{
	reverse(vPlace.begin(), vPlace.end());
}

void SP_ExportPRISM::Random(vector<wxString>& vPlace)
{
	std::random_shuffle(vPlace.begin(), vPlace.end());
}

vector<wxString> SP_ExportPRISM::Noack1()
{
	InitNoack1();
...
vector<wxString> SP_ExportPRISM::Noack2()
{
	InitNoack2();
...
vector<wxString> SP_ExportPRISM::OrderNoack1()
{
...
float SP_ExportPRISM::CalcWeightforNoack1(unsigned idx)
{
	float res = 0.0;
...
void SP_ExportPRISM::InitNoack1()
{
	SP_Place* m_pSP_Place;
	PlaceIdMap::iterator pIt;
	m_vPlaces.clear();
...
void SP_ExportPRISM::AssignedforNoack1(unsigned nr, unsigned idx)
{
...
unsigned SP_ExportPRISM::FindIdxOfMax(const vector<float>& v, const vector<bool>& used)
{
	unsigned sz = v.size();
	unsigned res = 0;
	float foundMax = 0;
...
void SP_ExportPRISM::InitNoack2()
{
	SP_Place* m_pSP_Place;
	PlaceIdMap::iterator pIt;
	m_vPlaces.clear();
...
float SP_ExportPRISM::calcWeightforNoack2(unsigned idx)
{
	float res = 0.0;
	unsigned nrCounted = 0;
	SP_Transition* t;
...
void SP_ExportPRISM::AssignedforNoack2(unsigned nr, unsigned idx)
{
	m_vPlacesUsed[idx] = true;
	list<SP_DiscrArc*>* aList;
	list<SP_DiscrArc*>::iterator aIt;
...
vector<wxString> SP_ExportPRISM::OrderNoack2()
{
	vector<SP_Place*> l_vLocalPlaces;
	vector<wxString> m_vplace;
	m_vplace.clear();
...
bool SP_ExportPRISM::FromFile(vector<wxString>& vOrderedPlace)
{
	wxTextFile file;
	vOrderedPlace.clear();
...
wxString SP_ExportPRISM::ReadConstant()
{
