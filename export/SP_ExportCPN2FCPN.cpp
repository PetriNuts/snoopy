...
bool SP_ExportCPN2FCPN::Write(SP_MDI_Doc* p_doc, const wxString& p_fileName)
{
...
bool SP_ExportCPN2FCPN::WriteNodeclass(SP_DS_Nodeclass* p_pcVal, wxXmlNode* p_pcRoot)
{
	...
	WritePlace((*l_Iter), m_pcElem);
		...
		{
			unsigned long l_mContTransitionCount = 0;
			l_mContTransitionCount += m_graph->GetNodeclass(SP_DS_CONTINUOUS_TRANS)->GetElements()->size();
			m_pcElem = new wxXmlNode(NULL, wxXML_ELEMENT_NODE, wxT("nodeclass"));
...
bool SP_ExportCPN2FCPN::WritePlace(SP_DS_Node* p_pcVal, wxXmlNode* p_pcRoot)
{
...
bool SP_ExportCPN2FCPN::WriteTransition(SP_DS_Node* p_pcVal, wxXmlNode* p_pcRoot)
{
	CHECK_POINTER(p_pcVal, return FALSE);
	CHECK_POINTER(p_pcRoot, return FALSE);
...
bool SP_ExportCPN2FCPN::AcceptsDoc(SP_MDI_Doc* p_doc)
{
	CHECK_POINTER(p_doc, return false);
	CHECK_POINTER(p_doc->GetGraph(), return false);
